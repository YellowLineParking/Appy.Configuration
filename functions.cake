...
public class ProjectTaskConfigurationManager
{
	readonly IDictionary<string, HashSet<string>> _taskConfigLookup = new Dictionary<string, HashSet<string>>()
	{
        ...
        { "BuildDocker"	, new HashSet<string>(new string[] { "App", "Package" }) },
    ...
    public bool CanBuildDocker(ProjectConfigDescriptor projectConfig) => CanDoTask("BuildDocker", projectConfig.Type);
...
public static class ProjectLoader
{
	public static ProjectContainerDescriptor Load(ICakeContext context, string configFilePath, string basePath, string configuration)
	{
		...
		foreach(var projectPair in configLookup)
		{
            ...
            // TODO Check if dotnet project or something else...
...
public static string BuildNugetPackagePath(this ICakeContext context, DirectoryPath artifactsPath, ProjectDescriptor project) =>
    $"{context.MakeAbsolute(artifactsPath).FullPath}/{project.Document.DotNet.PackageId}.nupkg";

public static string BuildDockerFilePath(this ICakeContext context, string basePath, ProjectConfigDescriptor projectConfig) =>
    $"{basePath}/{projectConfig.Name}/Dockerfile";
